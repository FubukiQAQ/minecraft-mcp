...
Esc 菜单里会多一个独立的「MCP Bridge 设置」按钮，点进去即可改配置，**不需要重启游戏**：

- **即时生效**：授权等级、命令模式与黑白名单、令牌、请求超时、性能护栏、事件缓冲。这些值都是每次请求 / 每次事件现场读取的，点「保存并应用」后下一次调用就按新值走。
- **自动重启 HTTP 桥**：`enabled` / `bindAddress` / `port` / `allowRemoteBinding` 属于监听参数，改动后会自动停掉旧桥再按新配置拉起。界面底部显示桥当前的真实地址，端口填 `0` 时能看到系统实际分配的端口。
- 界面上编辑的是配置的草稿，点「取消」或按 Esc 直接丢弃；「恢复默认」只重置配置项，**令牌会保留**，免得把已经配好的 MCP server 顶掉。
- 令牌那一行带「生成」和「复制」，复制直接写进剪贴板。

专用服务端没有客户端界面，仍然改 `config/mcpbridge.json` 后重启。
